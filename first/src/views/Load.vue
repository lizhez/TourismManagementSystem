<template>
  <div style="margin-bottom: 10px;">
    ...
    <div style="margin: 10px 0">
      <el-input v-model="search" placeholder="输入旅游项目查询" style="width: 20%" size="mini" clearable></el-input>
      ...
      <router-link :to="'/main/user/newItem/'+id">
        <el-button size="mini" style="float: right;">发布新项目</el-button>
      </router-link>
    </div>
    ...
    <div v-for="(item,index) in tableData" :key="index">
      <el-card shadow="hover" :body-style="{ padding: '15px' }" style="height: 180px;">
        ...
        <div style="margin-top: 10px;">
          ...
          <el-popconfirm title="确定删除此项目吗？这样会彻底删除此项目！" @confirm="delArt(item.id)">
            <template #reference>
              <el-button size="small" type="danger" style="float: right;" v-if="statusItem===0||statusItem===-1">删除</el-button>
            </template>
          </el-popconfirm>
          <el-button  style="float: right;margin-right: 10px; "  size="small" @click="editTravel(item.id)">编辑</el-button>
          <el-button  style="float: right;margin-right: 10px; "  size="small" @click="to(item.id)" v-if="item.active===1">查看详情</el-button>
        </div>
      </el-card>
...
export default {
  ...
  methods:{
    ...
    editTravel(id) {
      this.$router.push("/main/user/editItem/"+id)
    },

    delArt(id){
      axios.delete("/api/travels/real/"+id).then(
          res=>{
            if(res.data.flag){
              this.$message({
                type: "success",
