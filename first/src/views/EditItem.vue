    ...
    <div>
      <el-form ref="editForm" status-icon :model="editForm" :rules="rules1"  label-width="80px">
        ...
        <el-form-item style="margin-bottom: 20px">
          <el-button type="primary" @click="toSubmit('editForm')">修改</el-button>
        </el-form-item>
      </el-form>
    </div>
  </div>
    ...
    methods: {
      ...
      load(){
        axios.get('/api/travels/' + this.id).then(res=>{
          ...
          console.log("pay",this.editForm)
          let MarkdownIt = require("markdown-it")
          let md = new MarkdownIt()
          this.editForm.context = md.render(res.data.data.context)
        })
      },
      toSubmit(formName) {
        this.$refs[formName].validate(
            valid => {
              if (valid) {
                this.editForm.uid=JSON.parse(sessionStorage.getItem("user")).id
                console.log(this.editForm)
                axios.put("/api/travels",this.editForm).then(
                    res=>{
                      if(res.data.flag){
                        this.$message({
                          type: "success",
